CREATE OR REPLACE FUNCTION T0026.ZANSU_THISMONTH(
...
BEGIN
	...
	BEGIN
		SELECT
		NVL(当月入庫＿ケース,0)-NVL(当月出庫＿ケース,0) + NVL(過不足数量＿ケース,0),--y.koba 過不足追加
		NVL(当月入庫＿ボール,0)-NVL(当月出庫＿ボール,0) + NVL(過不足数量＿ボール,0),--y.koba 過不足追加
		NVL(当月入庫＿バラ,0)-NVL(当月出庫＿バラ,0) + NVL(過不足数量＿バラ,0)--y.koba 過不足追加
	...
	if GETSUMATSU_SU_bo is null then
 	...
 	if zaikokb = 0 then
		RETURN GETSUMATSU_SU_c;
 	elsif zaikokb = 1 then
		RETURN GETSUMATSU_SU_b;
 	else
